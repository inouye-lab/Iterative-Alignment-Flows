...
__all__ = ['NaiveBayesClassifierDestructor',
           'RotatedNaiveBayesClassifierDestructor']


class NaiveBayesClassifierDestructor(ClassifierDestructor):
...
class RotatedNaiveBayesClassifierDestructor(NaiveBayesClassifierDestructor):
    def __init__(self, rotation_estimator=None, **kwargs):
        super().__init__(**kwargs)
        self.rotation_estimator = rotation_estimator
    
    def _fit_pre_transform(self, X, y=None):
        # Fit SIE and extract W
        if self.rotation_estimator is None:
            rot_est = SingleIndexEnsembleEstimator()
        else:
            rot_est = clone(self.rotation_estimator)
        rot_est.fit(X, y)
        # Extract rotation W and create transformer
        W = rot_est.rotation_
        self.pre_transformer_ = LinearProjector.create_fitted(
            A=W, b=np.zeros(W.shape[0]))
        return self.pre_transformer_
        
    def _fit_post_transform(self, X, y=None):
        d_bary_inv = super()._fit_post_transform(X, y)
        self.post_transformer_ = CompositeDestructor.create_fitted([
            d_bary_inv,
            create_inverse_transformer(self.pre_transformer_)
        ])
        return self.post_transformer_
